...
import java.util.Optional;import json

class JsonQbftConfigOptions(JsonBftConfigOptions, QbftConfigOptions):
    DEFAULT = JsonQbftConfigOptions(json.loads('{}'))
    VALIDATOR_CONTRACT_ADDRESS = 'validatorcontractaddress'
    START_BLOCK = 'startblock'

    def __init__(self, bftConfigRoot):
        super().__init__(bftConfigRoot)

    def getValidatorContractAddress(self):
        return self.bftConfigRoot.get(VALIDATOR_CONTRACT_ADDRESS, '').lower()

    def getStartBlock(self):
        return self.bftConfigRoot.get(START_BLOCK, None)

    def asMap(self):
        map = super().asMap()
        map[VALIDATOR_CONTRACT_ADDRESS] = self.getValidatorContractAddress()
        map[START_BLOCK] = self.getStartBlock()
        return map
